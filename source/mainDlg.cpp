...
QRecursiveMutex gethostbyaddrThreadCS;
                             ...
                             pjsip_rx_data* rdata)
{
    ...
    if (busy) {
        // 486 Busy Here
        onyx_call_busy(call_info->id, _T("Call already exists"));
        ...
        // 486 Busy Here
        onyx_call_busy(call_info->id, _T("Active calls limit"));
        user_data->hidden = true;
    }
            ...
            if (accountSettings.autoAnswer == _T("all")) {
                ...
                if (!autoAnswer) {
                    ...
                    if (hsr) {
                        ...
                        if (callInfoValue.contains(_T("auto answer"))) {
                            autoAnswer = true;
                        }
                        else {
                            CAtlRegExp<> regex;
...
static void on_nat_detect(const pj_stun_nat_detect_result * res)
{
    if (res->status != PJ_SUCCESS) {
        ...
        if (res->nat_type == PJ_STUN_NAT_TYPE_SYMMETRIC) {
            QString message = _T("The softphone may not work properly with enabled STUN and your internet connection.");
            QMetaObject::invokeMethod(mainDlg, "BaloonPopup", Qt::QueuedConnection,
                                      Q_ARG(QString, Translate(_T("Symmetric NAT detected!"))),
...
static void on_pager2(pjsua_call_id call_id, const pj_str_t * from, const pj_str_t * to, const pj_str_t * contact, const pj_str_t * mime_type, const pj_str_t * body, pjsip_rx_data * rdata, pjsua_acc_id acc_id)
{
    ...
    //--
    number->setString(Onyx::BuildSIPURI(&sipuri));
                                    ...
                                    pj_bool_t * p_cont)
{
    ...
    str->sprintf("%s: %s",
        Translate(_T("Call Transfer")).toLatin1().constData(),
        Onyx::PjToStr(status_text, TRUE).toLatin1().constData());
    if (final) {
        str->append(QString(" [%1]").arg(Translate(_T("Final"))));
    }
...
void CmainDlg::OnDestroy()
{
    ...
    WTSUnRegisterSessionNotification(winId());
    ...
    : QWidget(parent)
{
    ...
    QString audioCodecsCaptions = _T("opus/48000/2;Opus 24 kHz;"
...
{
    WTSRegisterSessionNotification(winId(), NOTIFY_FOR_THIS_SESSION);
...
{
    onMWIInfo(0, 0); // voicemail button update
    SetPaneText2();
    setWindowTitle(_T(_GLOBAL_NAME_VISIBLE));
    UpdateWindowText();
    pageDialer->SetName();
}
...
void CmainDlg::ShowTrayIcon()
{
    ...
    trayIcon->show();
    trayIcon->setToolTip(_T(_GLOBAL_NAME_VISIBLE));
}
...
void CmainDlg::OnCreated()
{
    ...
    if (lstrlen(theApp.m_lpCmdLine)) {
        CommandLine(QString::fromWCharArray(theApp.m_lpCmdLine));
...
{
    if (trayIcon) {
        QString tip = _T(_GLOBAL_NAME_VISIBLE);
        if (accountSettings.accountId) {
...
void CmainDlg::BaloonPopup(QString title, QString message, QSystemTrayIcon::MessageIcon icon)
{
    if (trayIcon) {
...
void CmainDlg::MainPopupMenu(bool isMenuButton)
{
    ...
    switch (id) {
    case ID_ACCOUNT_ADD: OnMenuAccountAdd(); break;
    case ID_ACCOUNT_EDIT_RANGE ... ID_ACCOUNT_EDIT_RANGE + 99: OnMenuAccountEdit(id); break;
    case ID_ACCOUNT_CHANGE_RANGE ... ID_ACCOUNT_CHANGE_RANGE + 99: OnMenuAccountChange(id); break;
    ...
    default: if (id >= ID_CUSTOM_RANGE && id <= ID_CUSTOM_RANGE + 99) OnMenuCustomRange(id); break;
    }
}
// === Message Handlers ===
...
void CmainDlg::onCallState(pjsua_call_info* call_info, call_user_data* user_data)
{
    ...
    if (call_info->state == PJSIP_INV_STATE_CONFIRMED) {
        ...
        if (!user_data->commands.isEmpty()) {
            startTimer((UINT_PTR)call_info->id, 1000, [=]() { DTMFQueueTimerHandler(); });
        }
    }
    ...
    if (call_info->role == PJSIP_ROLE_UAC) {
        if (call_info->last_status == 180 && !call_info->media_cnt) {
            if (toneCalls.isEmpty()) {
                QTimer::singleShot(4500, this, [=]() { onPlayerPlay(ONYX_SOUND_RINGING, 0); });
...
void CmainDlg::onIncomingCall(pjsua_call_info* call_info, call_user_data* user_data)
{
    ...
    if (user_data->forwarding && messagesDlg->CallAction(ONYX_ACTION_FORWARD, "", call_info->id)) {
        ...
        if (messagesDlg->GetCallsCount()) {
            ...
            if (!accountSettings.ringtone.length()) {
                onPlayerPlay(ONYX_SOUND_RINGTONE, 0);
            } else {
                onPlayerPlay(ONYX_SOUND_CUSTOM, (LPARAM)&accountSettings.ringtone);
            }
        }
        if (accountSettings.headsetSupport) {
            Hid::SetRing(true);
    ...
    if (accountSettings.localDTMF && playBeep) {
        onPlayerPlay(ONYX_SOUND_RINGIN2, 0);
    }
...
void CmainDlg::MessagesIncoming(QString* number, QString* message, CTime* pTime)
{
    ...
    if (messagesContact) {
        ...
        onPlayerPlay(ONYX_SOUND_MESSAGE_IN, 0);
    }
}
...
void CmainDlg::timerEvent(QTimerEvent* event)
{
    ...
    if (id == IDT_TIMER_AUTOANSWER) {
        ...
        onPlayerPlay(ONYX_SOUND_RINGING, 0);
    }
}
...
{
    QString title = _T(_GLOBAL_NAME_VISIBLE);
    QString titleAdder;
    if (!accountSettings.account.label.isEmpty())
...
void CmainDlg::PJAccountDelete(bool deep, bool exit, QByteArray code)
{
    Unsubscribe();
...
void CmainDlg::OnUpdateWindowText(int mode, int unused)
{
    if (mode == 1) {
...
void CmainDlg::UpdateWindowText(QString text, int icon, bool afterRegister)
{
    ...
    if (isOffline) {
        icon = IDI_DEFAULT;
        if (MACRO_ENABLE_LOCAL_ACCOUNT) str = _T(_GLOBAL_NAME_VISIBLE);
...
LRESULT CmainDlg::onCopyData(WPARAM wParam, LPARAM lParam)
{
    ...
    if (s) {
        QString params = QString::fromWCharArray((LPCWSTR)s->lpData);
        if (s->dwData == 1) return CommandLine(params);
        else if (s->dwData == 2) {
            QString* str = new QString(params);
            QMetaObject::invokeMethod(this, "onCommandLine", Qt::QueuedConnection, Q_ARG(QString*, str));
            return FALSE;
        }
    }
    return TRUE;
...
void CmainDlg::DialNumberFromCommandLine(QString params)
{
...
void CmainDlg::DialNumber(const QString& params)
{
    ...
    number.replace("%20", " ").replace("%2B", "+").trimmed();
    if (!number.isEmpty()) {
        if (message.isEmpty()) {
...
bool CmainDlg::MakeCall(QString number, bool hasVideo, bool fromCommandLine, bool noTransform, QString name)
{
    if (accountSettings.singleMode && messagesDlg->GetCallsCount()) {
        GotoTab(0);
        return false;
    }
    if (!pjsua_acc_is_valid(account) && !accountSettings.enableLocalAccount && Onyx::IsPSTNNumber(number) && !Onyx::IsIP(number)) {
        Account dummy;
...
bool CmainDlg::MessagesOpen(QString number, bool forCall, bool noTransform, QString name)
{
    QString commands;
...
void CmainDlg::ShortcutAction(Shortcut* shortcut, bool block, bool second)
{
    ...
    if (shortcut->type == ONYX_SHORTCUT_CALL) {
        ...
        QMessageBox::information(this, "", _T(_GLOBAL_BUSINESS_FEATURE));
...
LRESULT CmainDlg::onPlayerPlay(int soundId, LPARAM lParam)
{
    QString filename;
    BOOL noLoop = FALSE;
    BOOL inCall = FALSE;
    if (soundId == ONYX_SOUND_CUSTOM) {
        filename = *(QString*)lParam;
...
static PJ_DEF(pj_status_t) on_pjsua_wav_file_end_callback(pjmedia_port*, void* args)
{
    QMetaObject::invokeMethod(mainDlg, "onPlayerStop", Qt::QueuedConnection, Q_ARG(int, 0), Q_ARG(int, 0));
    return -1;
}

void CmainDlg::PlayerPlay(const QString& filename, bool noLoop, bool inCall)
{
    PlayerStop();
    if (!filename.isEmpty()) {
...
bool CmainDlg::CommandCallPickup(QString number)
{
...
LRESULT CmainDlg::onShellHookMessage(WPARAM wParam, LPARAM lParam)
{
    if (wParam == HSHELL_APPCOMMAND) {
        ...
        if (nCmd == APPCOMMAND_MEDIA_PLAY || nCmd == APPCOMMAND_MEDIA_PLAY_PAUSE || nCmd == APPCOMMAND_MEDIA_STOP) {
            if (!ringinDlgs.isEmpty()) {
                RinginDlg* dlg = ringinDlgs.first();
                if (nCmd == APPCOMMAND_MEDIA_STOP) dlg->OnBnClickedDecline();
                else dlg->CallAccept(dlg->remoteHasVideo);
            } else {
                if (nCmd == APPCOMMAND_MEDIA_STOP) messagesDlg->OnBnClickedEnd();
                else {
                    QPushButton* callButton = pageDialer->findChild<QPushButton*>("callButton");
                    if (callButton && callButton->isVisible() && callButton->isEnabled()) pageDialer->OnBnClickedCall();
                    else messagesDlg->OnBnClickedHold();
                }
            }
        } else if (nCmd == APPCOMMAND_MEDIA_PAUSE) messagesDlg->OnBnClickedHold();
    }
    return 0;
}
                ...
                if (user_data) {
                    ...
                    if (user_data->autoAnswer && !accountSettings.bringToFrontOnIncoming) {
                        ...
                        if (QApplication::activeWindow() != this) {
                            ...
                            BaloonPopup(Translate("Auto Answer"), name, NIIF_INFO);
                        }
                    }
                    user_data->CS.Unlock();
                }
...
{
    QDesktopServices::openUrl(QUrl(_T(_GLOBAL_MENU_WEBSITE)));
}
...
LRESULT CmainDlg::onUsersDirectoryLoaded(URLGetAsyncData* response)
{
    ...
    if (!message.isEmpty() && !usersDirectorySilent)
        BaloonPopup(Translate("Directory of Users"), message, NIIF_INFO);
    delete response;
    return 0;
}
...
#ifdef _GLOBAL_VIDEO
int CmainDlg::VideoCaptureDeviceId(QString name)
{
    // ...
}
