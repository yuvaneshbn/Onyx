...
JumpList::JumpList(const QString& AppID) :
pcdl(0)
{
	...
	if (hinstDll) {
		...
		if (pSetCurrentProcessExplicitAppUserModelID) {
			pSetCurrentProcessExplicitAppUserModelID(AppID.toStdWString().c_str());
			HRESULT hr = CoCreateInstance(CLSID_DestinationList, NULL, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&pcdl));
			if (SUCCEEDED(hr)) {
				pcdl->SetAppID(AppID.toStdWString().c_str());
			}
		}
...
void JumpList::AddTasks()
{
	if (pcdl)
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			{
				IShellLinkW * psl;
				const QString exitText = Translate(TEXT("Exit"));
				hr = _CreateShellLink(L"/exit", exitText.toStdWString().c_str(), &psl, IDI_EXIT);
				if (SUCCEEDED(hr))
				{
...
max_items_jumplist(100)
{
    ...
    pcdl->SetAppID(m_AppID.toStdWString().c_str());
}
...
HRESULT JumpList::_CreateShellLink(PCWSTR pszArguments, PCWSTR pszTitle, IShellLink **ppsl, int iconindex, bool WA)
{
    ...
    if (SUCCEEDED(hr))
    {
        psl->SetIconLocation(path.toStdWString().c_str(), iconindex);
        if (WA)
        {
            std::wstring fname;
            fname.resize(MAX_PATH);
...
bool JumpList::CreateJumpList(const QString& pluginpath, const QString& pref, const QString& fromstart, 
            ...
            bool frequent, bool tasks, bool addbm, bool playlist, const QString& bms)
{
    ...
    if (!bms.isEmpty() && addbm && hr == S_OK)
    {
        std::wstringstream ss(bms.toStdWString());
        ...
        while (getline(ss, line1))
        {
            ...
            {				
                ...
                if (!_IsItemInArray(QString::fromStdWString(line2), poaRemoved))
                {
...
HRESULT JumpList::_AddTasksToList()
{
    ...
    {
        std::wstring args;
        args = path.toStdWString() + L",_pref@0";;

        IShellLink * psl;
        hr = _CreateShellLink(args.c_str(), s1.toStdWString().c_str(), &psl, 0, false);
        if (SUCCEEDED(hr))
        {
            hr = poc->AddObject(psl);
            psl->Release();
        ...
        args = path.toStdWString() + L",_openfile@0";

        if (SUCCEEDED(hr))
        {
            hr = _CreateShellLink(args.c_str(), s4.toStdWString().c_str(), &psl, 1, false);
            if (SUCCEEDED(hr))
            {
                hr = poc->AddObject(psl);
                psl->Release();
        ...
        args = path.toStdWString() + L",_resume@0";

        if (SUCCEEDED(hr))
        {
            hr = _CreateShellLink(args.c_str(), s3.toStdWString().c_str(), &psl, 3, false);
            if (SUCCEEDED(hr))
            {
                hr = poc->AddObject(psl);
                psl->Release();
        ...
        args = path.toStdWString() + L",_fromstart@0";

        if (SUCCEEDED(hr))
        {
            hr = _CreateShellLink(args.c_str(), s2.toStdWString().c_str(), &psl, 2, false);
            if (SUCCEEDED(hr))
            {
                hr = poc->AddObject(psl);
                psl->Release();
...
bool JumpList::_IsItemInArray(const QString& path, IObjectArray *poaRemoved)
{
    bool fRet = false;
    UINT cItems;
    if (SUCCEEDED(poaRemoved->GetCount(&cItems)))
    {
        ...
        for (UINT i = 0; !fRet && i < cItems; i++)
        {
            if (SUCCEEDED(poaRemoved->GetAt(i, IID_PPV_ARGS(&psiCompare))))
            {
                ...
                fRet = (QString::fromStdWString(removedpath) == path);
...
HRESULT JumpList::_AddCategoryToList()
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        hr = pcdl->AppendCategory(s5.toStdWString().c_str(), poa);
        poa->Release();
    }
...
HRESULT JumpList::_AddCategoryToList2()
{
    ...
    if(AGAVE_API_PLAYLISTS && AGAVE_API_PLAYLISTS->GetCount()){
        for(size_t i = 0; i < AGAVE_API_PLAYLISTS->GetCount(); i++){
            ...
            title = QString::fromStdWString(AGAVE_API_PLAYLISTS->GetName(i));
            ...
            title += QString::fromStdWString(tmp);
            
            hr = _CreateShellLink(AGAVE_API_PLAYLISTS->GetFilename(i), title.toStdWString().c_str(), &psl, 5, true);
            if (SUCCEEDED(hr))
            {
    ...
    if (SUCCEEDED(hr))
    {
        // Add the category to the Jump List.  If there were more categories, they would appear
        // from top to bottom in the order they were appended.
        hr = pcdl->AppendCategory(s6.toStdWString().c_str(), poa);
        poa->Release();
    }
...
{
    IObjectArray *poa;
    padl->SetAppID(m_AppID.toStdWString().c_str());
    ...
    if (SUCCEEDED(hr))
    {
        ...
        {   
            ...
            pad->SetAppID(m_AppID.toStdWString().c_str());
