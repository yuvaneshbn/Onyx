...
MainWindow::MainWindow(const QString& my_id,
    ...
    // Start muted / no transmit until a target is selected.
    audio_->setTxMuted(true);
    audio_->setAllowTx(false);
...
void MainWindow::refreshParticipants(bool silent) {
    ...
    // Auto-broadcast to all peers by default on initial join (do not override manual controls).
    QSet<QString> auto_targets = participant_set;
    auto_targets.remove(my_id_);
    if (!has_auto_broadcasted_ && !auto_targets.isEmpty()) {
        has_auto_broadcasted_ = true;
        if (targets_.isEmpty()) {
...
void MainWindow::updateTargets() {
    if (!targets_.isEmpty()) {
        ...
        for (const auto& t : targets_) {
            if (t == my_id_) continue;
            auto ep = getClientEndpoint(t.toStdString());
            dests.push_back(ep);
            qDebug() << "[MainWindow] Resolved destination for target" << t << "->" << ep.c_str();
        }
        if (!audio_->isRunning()) {
            audio_->start("127.0.0.1");
        }
        audio_->updateDestinations(dests);
        audio_->setAllowTx(true);
    } else {
        if (audio_->isRunning() && !stop_capture_timer_.isActive()) {
