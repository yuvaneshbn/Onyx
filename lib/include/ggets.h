...
// Keep original error codes
enum {FGGETS_OK = 0, FGGETS_NOMEM};

template<class TTCHAR>
int fggets(TTCHAR** ln, QTextStream* f, bool& bOwnBuffer, const int nDefaultBufferSizeChars = 0);

// Keep the implementation available to translation units that include this header.
#include "ggets.cpp"

//#define ggets(ln) fggets(ln, stdin)   // can be defined later if needed
